...
def zeroed_observation(observation):
    ...
    if hasattr(observation, 'shape'):
        return torch.zeros(observation.shape)
...
class SequentialMemory(Memory):
    ...
    def sample(self, batch_size, batch_idxs):
        ...
        for idx in batch_idxs:
            ...
            action = [self.actions[i] for i in range(idx-1- self.window_length, idx)]
            reward = [self.rewards[i] for i in range(idx-1- self.window_length, idx)]
            terminal1 = [self.terminals[i] for i in range(idx-1- self.window_length, idx)]
...
class EpisodicMemory(Memory):
    def __init__(self, n_max_episode=10000, n_max_episode_len=None, memory_for_steps=None):
        ...
        self.n_max_episode_len = n_max_episode_len
    ...
    def append_episode(self, e_idx, len):
        """
        :param e_idx: ending index
        :param len: episode length
        :return: None
        """
        self.memory_for_episode.append((e_idx, len))

    def sample(self, batch_size, n_current_episode, batch_idxs=None):
        if batch_idxs is None:
            # Draw random indexes such that we have enough entries before each index to fill the
            # desired window length.
            batch_idxs = sample_batch_indexes(0, n_current_episode-1, size=batch_size)
        batch_idxs = np.array(batch_idxs) + 1
        e = []
        for idx in batch_idxs:
            e_idx, length = self.memory_for_episode[idx]
            self.memory_for_steps.window_length = length
            e.append(self.memory_for_steps.sample(batch_size=1, batch_idxs=[e_idx]))

        return e

    def append(self, observation, action, reward, terminal, training=True):
        self.memory_for_steps.append(observation, action, reward, terminal, training=training)
