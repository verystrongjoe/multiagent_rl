...
class Trainer:
    ...
    def get_exploration_action(self, states):
        """
        ...
        states = list(states)  # deque to list
        for i in range(len(states)):  # remove first axis
            states[i].squeeze_()

        margin = 4 - len(states)  # margin to rpad with last obs
        stacked_observation = torch.cat([states[k] for k in range(len(states))] + [states[-1]]*margin, 1)  # stacked obs

        stacked_observation = torch.unsqueeze(stacked_observation, 0)

        stacked_observation = stacked_observation.to(self.device)

        action, _ = self.actor.forward(stacked_observation)
        action = action.detach()
    ...
    def optimize(self):
        ...
        margin = 4 - len(s1)  # margin to rpad with last obs
        #s1 = torch.cat([s1[k] for k in range(len(s1))] + [s1[-1]] * margin, 2)  # stacked obs
        s1 = [torch.cat(s1[i], 1) for i in range(len(s1))]

        margin = 4 - len(s2)  # margin to rpad with last obs
        # s2 = torch.cat([s2[k] for k in range(len(s2))] + [s2[-1]] * margin, 1)  # stacked obs
        s2 = [torch.cat(s2[i], 1) for i in range(len(s2))]


        # todo : just we could remove this type casting with creating tensors itself when it is creates
        s1 = torch.stack(s1)
        s2 = torch.stack(s2)
        a1 = torch.stack(a1)
        r1 = torch.stack(r1)
        ...
        loss_actor += F.smooth_l1_loss(pred_s2, s2[:,:,-10:]) * 0.1
